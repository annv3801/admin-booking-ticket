...
const ViewDetailFilm = () => {
    ...
    const fetchMovies = async () => {
        try {
            ...
            setUpdatedSlug(response.data.data?.slug || '');
            setUpdatedGroup(response.data.data?.groupEntity.id || 0);
            setUpdatedDescription(response.data.data?.description || '');
            setUpdatedDirector(response.data.data?.director || '');
            setUpdatedActor(response.data.data?.actor || '');
            setUpdatedPremiere(response.data.data?.premiere || '');
            setUpdatedDuration(response.data.data?.duration || 0);
            setUpdatedLanguage(response.data.data?.language || '');
            setUpdatedRated(response.data.data?.rated || '');
            setUpdatedTrailer(response.data.data?.trailer || '');
            setUpdatedTotalRating(response.data.data?.totalRating || '');
