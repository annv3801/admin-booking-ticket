...
const ViewListGroup = () => {
    ...
    const fetchMovies = async () => {
        try {
            const response = await axios.post(`https://cinema.dummywebsite.me/Group/View-List-Groups-Not-Have-Type`, {
                pageSize,
                currentPage,
            });
    ...
    const handleDelete = (id) => {
        axios.delete("https://cinema.dummywebsite.me/Group/delete-Group/" + id, config)
            .then(res => {
                if (res.status === 200) {
                        ...
                        <table className="table align-middle gs-0 gy-4">
                            ...
                            {movies?.data?.data.map((movie) => (
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" className="text-dark fw-bold text-hover-primary d-block mb-1 fs-6">{movie.index}</a>
                                    </td>
                                    <td>
                                        <span className="badge badge-light-success">{movie.status}</span>
                                    </td>
                                    <td className="text-end">
                                        <NavLink to={`/Group/${movie.id}`} className="btn btn-bg-light btn-color-muted btn-active-color-primary btn-sm px-4 me-2">View</NavLink>
